...
def compare_assignments():
    """Handles comparison between two selected assignments, including metadata."""
    ...
    # Collect metadata for both assignments
    metadata1 = collect_metadata(file_path1, text1)
    metadata2 = collect_metadata(file_path2, text2)

    # Simple metadata comparison
    metadata_diff = {}
    time1 = datetime.strptime(metadata1['submission_time'], "%Y-%m-%d %H:%M:%S")
    time2 = datetime.strptime(metadata2['submission_time'], "%Y-%m-%d %H:%M:%S")
    time_diff = abs((time1 - time2).total_seconds()) / 60  # Difference in minutes
    metadata_diff['time_diff'] = f"{time_diff:.2f} minutes"
    metadata_diff['time_suspicious'] = time_diff < 5  # Flag if less than 5 minutes apart
    metadata_diff['word_count_diff'] = abs(metadata1['word_count'] - metadata2['word_count'])
    metadata_diff['file_size_diff'] = abs(metadata1['file_size'] - metadata2['file_size'])
